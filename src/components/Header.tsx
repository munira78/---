...
const Header: React.FC = () => {
  ...
  const defaultLinkClass = `${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'} transition-colors duration-300`;
  const hoverClass = `hover:text-amber-400`;
            ...
            <div className="ml-10 flex items-baseline space-x-4 rtl:space-x-reverse">
              {navLinks.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) => `${isActive ? 'border-b-2' : hoverClass} ${defaultLinkClass} px-3 py-2 rounded-md text-sm font-medium`}
                >
                  {link.label}
                </NavLink>
              ))}
              {isAuthenticated ? (
                 <button onClick={handleLogout} className={`${defaultLinkClass} ${hoverClass} px-3 py-2 rounded-md text-sm font-medium`}>تسجيل الخروج</button>
              ) : (
                <NavLink to="/login" style={({ isActive }) => (isActive ? activeStyle : undefined)} className={({ isActive }) => `${isActive ? 'border-b-2' : hoverClass} ${defaultLinkClass} px-3 py-2 rounded-md text-sm font-medium`}>ادمن</NavLink>
              )}
            </div>
          </nav>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <NavLink
                ...
                style={({ isActive }) => (isActive ? mobileActiveStyle : undefined)}
                className={({ isActive }) => `${isActive ? `${mobileActiveBg}` : mobileDefaultText} block px-3 py-2 rounded-md text-base font-medium`}
              >
                {link.label}
              </NavLink>
            ))}
             {isAuthenticated ? (
                ...
                <NavLink to="/login" onClick={() => setIsMenuOpen(false)} style={({ isActive }) => (isActive ? mobileActiveStyle : undefined)} className={({ isActive }) => `${isActive ? `${mobileActiveBg}` : mobileDefaultText} block px-3 py-2 rounded-md text-base font-medium`}>ادمن</NavLink>
              )}
          </div>
        </div>
      )}
