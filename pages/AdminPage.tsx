        ...
        <div className="space-y-2">
          {configForm.quickLinks.map(link => (
            <div key={link.id} className="grid grid-cols-12 gap-2 items-center">
              <input value={link.label} onChange={e => handleQuickLinkChange(link.id, 'label', e.target.value)} placeholder="النص" className={`${inputClass} col-span-5`} />
              <input value={link.url} onChange={e => handleQuickLinkChange(link.id, 'url', e.target.value)} placeholder="الرابط" className={`${inputClass} col-span-5`} />
              <button type="button" onClick={() => removeQuickLink(link.id)} className="col-span-2 bg-red-600 text-white p-2 rounded hover:bg-red-500">حذف</button>
            </div>
          ))}
        </div>
          ...
          <div key={year}>
            ...
            {curriculumForm[year].map((course, index) => (
              <div key={course.id} className="grid grid-cols-12 gap-2 items-center">
                <input value={course.code} onChange={e => handleCourseChange(year, index, 'code', e.target.value)} placeholder="الرمز" className={`${inputClass} col-span-2`} />
                <input value={course.name} onChange={e => handleCourseChange(year, index, 'name', e.target.value)} placeholder="اسم المادة" className={`${inputClass} col-span-6`} />
                <input type="number" value={course.credits} onChange={e => handleCourseChange(year, index, 'credits', parseInt(e.target.value) || 0)} placeholder="الساعات" className={`${inputClass} col-span-2`} />
                <button type="button" onClick={() => removeCourse(year, course.id)} className="col-span-2 bg-red-600 text-white p-2 rounded hover:bg-red-500">حذف</button>
              </div>
            ))}
            </div>
