...
const NewsPage: React.FC = () => {
  ...
  return (
    <div>
      <h1 className={`text-4xl font-bold text-amber-400 mb-8 pb-4 border-b-2 ${borderColor}`}>الأخبار والأنشطة</h1>
      {news.length > 0 ? (
        <div className="space-y-8">
          {news.map((article) => (
            <div key={article.id} className={`${cardBg} p-6 rounded-lg border-l-4 border-blue-400 shadow-lg hover:border-amber-400 transition-colors duration-300 border ${borderColor}`}>
              {article.imageUrl ? (
